...
import { ArrowLeft, Edit2, Save, X, Activity, Flame, Battery } from "lucide-react";
...
export default function InbodyProfilePage() {
  ...
  // 2. 상태 관리 (개선사항 & 루틴)
  const [improvements, setImprovements] = useState(""); // 줄바꿈으로 구분
  const [routine, setRoutine] = useState({
    Mon: "", Tue: "", Wed: "", Thu: "", Fri: "", Sat: "", Sun: ""
  });

  // 수정 모드 토글 (어떤 섹션을 수정 중인지)
  ...
  // 저장 핸들러 (Profile & Today)
  const saveProfile = async () => {
    // 날짜가 있으면 update, 없으면 insert (Upsert)
              ...
              <div key={item.key} className="bg-black/20 rounded-2xl p-4">
                ...
                {editing === 'profile' ? (
                  <input 
                    ...
                    onChange={(e) => setProfile({...profile, [item.key]: parseFloat(e.target.value)})}
                    className="w-full bg-white/10 text-center text-white text-lg rounded py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                ) : (
                  <p className="text-2xl font-light text-white">
                    {profile[item.key as keyof typeof profile]} <span className="text-xs text-white/30">{item.unit}</span>
                  </p>
                )}
              </div>
            ))}
              ...
              <div key={item.key} className="flex justify-between items-center border-b border-white/5 pb-2">
                ...
                {editing === 'today' ? (
                  <input 
                    ...
                    value={profile[item.key as keyof typeof profile]} 
                    onChange={(e) => setProfile({...profile, [item.key]: parseFloat(e.target.value)})}
                    className="w-24 bg-white/10 text-right text-white rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                ) : (
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 min-h-[100px]">
            {editing === 'improvements' ? (
              <textarea 
                ...
                placeholder="1. Fix Thoracic Mobility&#10;2. Pelvic Tilt"
                className="w-full h-32 bg-transparent text-white/80 text-sm leading-relaxed focus:outline-none resize-none placeholder:text-white/20"
              />
            ) : (
            ...
            {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
              <div key={day} className="flex gap-4 p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                ...
                {editing === 'routine' ? (
                  <input 
                    type="text" 
                    value={routine[day as keyof typeof routine]} 
                    onChange={(e) => setRoutine({...routine, [day]: e.target.value})}
                    placeholder="Rest or Workout..."
                    className="flex-1 bg-white/5 text-sm text-white px-3 py-1 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                ) : (
                  <div className="flex-1">
                    {routine[day as keyof typeof routine] ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white/80">{routine[day as keyof typeof routine]}</span>
                      </div>
                    ) : (
